	...
	getCoreToBind(tasks, node, task_number, cpu){
			...
			if(this._options["threads_per_core"]===1){
				switch(this._options["distribution_core"]){
					case('block'):
						...
						socket = this._socket_arr[parseInt((task_number*this._options["cpu_per_task"]+cpu)/
					...
					case('fcyclic'):
						if(this._options["cores"]%this._options["cpu_per_task"]===0 || this._options["cpu_per_task"]%this._options["cores"]===0){
							...
							thread = parseInt((task_number*this._options["cpu_per_task"]+cpu)/
										(this._options["cores"]*this._options["sockets"]))%this._options["threads_per_core"];
							socket = this._socket_arr[parseInt((task_number*this._options["cpu_per_task"]+cpu)/
										this._options["cores"])%this._options["sockets"]];
						}else{
							core = (task_number*this._options["cpu_per_task"]+cpu)%this._options["cores"]; 
							thread = parseInt((task_number*this._options["cpu_per_task"]+cpu)/
										this._options["cores"])%this._options["threads_per_core"];
							socket = this._socket_arr[parseInt((task_number*this._options["cpu_per_task"]+cpu)/
										(this._options["cores"]*this._options["sockets"]))%this._options["sockets"]];
						}
						break;
				}
			}else{
			...
			if(this._options["threads_per_core"]!== 1){
				switch(this._options["distribution_core"]){
					...
					case('fcyclic'):
						...
						thread = parseInt(((parseInt(task_number/this._options["sockets"])*this._options["cpu_per_task"])+cpu)/(this._options["cores"]))%this._options["threads_per_core"];
						break;
				}
			}else{
				core = (parseInt(task_number/this._options["sockets"])*this._options["cpu_per_task"]+cpu)%this._options["cores"];
		...
		// Distribution-Socket Fcyclic
			...
			if(this._options["threads_per_core"]!==1){
				...
				switch(this._options["distribution_core"]){
					case('block'):
						core = parseInt(((task_number*this._options["cpu_per_task"])+cpu)/(this._options["threads_per_core"]*this._options["sockets"]));
						thread = parseInt(((task_number*this._options["cpu_per_task"])+cpu)/this._options["sockets"])%this._options["threads_per_core"];
						break;
					case('fcyclic'):
						core = parseInt(((task_number*this._options["cpu_per_task"])+cpu)/(this._options["sockets"]))%this._options["cores"];
						thread = parseInt((parseInt(((task_number*this._options["cpu_per_task"])+cpu)/
								(this._options["sockets"]))/this._options["cores"])%this._options["threads_per_core"])%this._options["threads_per_core"];
						break;
				}
			}else{
				core = parseInt(((task_number*this._options["cpu_per_task"])+cpu)/(this._options["sockets"]))%this._options["cores"];
